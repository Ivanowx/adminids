...
RegisterCommand("id", function()
    ESX.TriggerServerCallback("esx_marker:fetchUserRank", function(playerRank)
        if playerRank == "admin" or playerRank == "superadmin" then
          if not idovi then
            TriggerEvent("pNotify:SendNotification", {text = "ID's are turned on!", type = "success", queue = "success", timeout = 1500, layout = "center"})
            idovi = true
          else
            idovi = false
            TriggerEvent("pNotify:SendNotification", {text = "ID's are turned off!", type = "success", queue = "success", timeout = 1500, layout = "center"})
          end
        else
           TriggerEvent("pNotify:SendNotification", {text = "You don't have permission!", type = "success", queue = "success", timeout = 2000, layout = "center"})
        end
    end)
end)
